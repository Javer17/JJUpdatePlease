...
function ratingToValue(rating)
{
  ...
  for (const prefix of Object.keys(RATING_MAGNITUDES))
  {
    if (trimmed.startsWith(prefix))
    {
      ...
      if (suffix === 'D' || suffix === 'ID') { return magnitude }
      if (suffix === 'R' || suffix === 'IR') { return -magnitude }
    }
  }
...
function updateSenatePredictions(rawPredictionsText, predictionsText)
{
  ...
  {
    console.warn(`Expected ${stateCount * 2} updated rows (one D + one R per state) but found ${updatedCount} - check that every state in raw-discord-senate.csv has matching ${FORECAST_DATE} democrat/republican rows in discord-senate.csv.`)
  }
